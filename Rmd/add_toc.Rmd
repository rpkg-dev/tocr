...
```{r close_connections, comment = "#>", purl = FALSE}
...
path_to_wd <- switch(EXPR = Sys.info()[["user"]],
                     "salim" = "~/Code/Eigene Projekte/salim_b/RmdTOC",
                     NULL)
                                ...
                                filename)
{
  ...
  toc_begin_position <- integer(0)
  toc_end_position <- integer(0)
  ...
  if ( length(md_lines) > 0 )
  {
    ...
    {
      # skip everything wrapped in code fences
      if ( stringr::str_detect(string = md_lines[i],
                               pattern = dplyr::if_else(condition = code_chunk,
                                                        true = "^ {0,3}`{3,} *$",
                                                        false = "^ {0,3}`{3,}([^`]*)?$")) )
      {
  ...
  if ( length(md_lines) > 0 )
  {
    for ( i in 1:length(md_lines) )
    {
                                      ...
                                      pattern = "^ {0,3}#{1,6}\\s+\\S") )
      {
        ...
        anchor_links <-
          convert_header_to_anchor(header_text = header_text,
        ...
        # add header to TOC if of tier `min_tier` or lower
        if ( stringr::str_detect(string = md_lines[i],
                                 pattern = stringr::str_c("^ {0,3}#{", min_tier, ",6}\\s+\\S")) )
        {
...
#'   create TOC entries for all `<h2>` – `<h6>` headers.
...
)
{
  ...
  if ( position == -1 |
       (!add_title | markdown_flavor == "gitlab") & fallback_id_position == -1 )
  {
    add_backlinks <- FALSE
  }
  ...
  if ( nrow(toc_data) > 0 )
  {
    ...
    ## ... and use WORKAROUND if necessary
  ...
  {
    message("\nNo header entries found below `min_tier` (<h", min_tier, ">) in '", basename(md), "'! Nothing done.\n")
