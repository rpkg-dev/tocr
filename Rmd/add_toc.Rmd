...
evaluate_position <- function(position,
  ...
  } else rlang::abort(message = "This shouldn't be possible!")
...
convert_header_to_anchor <- function(header_text,
  ...
  if (!is.na(header_text)) {
    ...
    if (length(backlink_string) > 0L & isTRUE(!is.na(backlink_string))) {
      header_text %<>% dplyr::if_else(backlink_position == "after",
                                      paste0(., " [", backlink_string, "](dummylink)"),
                                      paste0("[", backlink_string, "](dummylink) ", .))
    }
...
process_md <- function(md_lines,
  ...
  toc_data <- dplyr::data_frame(tier = integer(),
                                enumeration = integer(),
...
- It was tried to adhere to the [CommonMark specification](http://commonmark.org/), i.e. to interpret the Markdown syntax the same way as the
...
add_toc <- function(md,
  ...
  for (i in seq_along(vars_to_check)) {
    ...
    if (!isTRUE(test_result)) {
      rlang::abort(message = paste0(
        "Invalid argument provided: ", names(vars_to_check)[i], "\n", test_result
      ))
    }
  }
  ...
  if (length(md) > 1L) {
    ...
    if (!isTRUE(test_result)) {
      rlang::abort(message = paste0(
        "Invalid argument provided: md\n", test_result, "\n`md` must be a character vector or alternatively a readable file path or URL (as a string)."
      ))
    }
  } else {
    ...
    if (!isTRUE(test_file) & !isTRUE(test_url)) {
      rlang::abort(message = paste0(
        ...
        "\n`md` must be a file path or a URL pointing to the Markdown document, or alternatively the Markdown document itself as a character vector."
      ))
    }
  }
  ...
  if (!isTRUE(test_result)) {
    rlang::abort(message = paste0(
      "Invalid argument provided: markdown_flavor\n", test_result
    ))
  }
  ...
  if (!isTRUE(test_result)) {
    rlang::abort(message = paste0(
      "Invalid argument provided: backlink_strings\n", test_result
    ))
  }
  ...
  if (!isTRUE(test_result)) {
    rlang::abort(message = paste0(
      "Invalid argument provided: backlink_position\n", test_result
    ))
  }
  ...
  if (!isTRUE(test_result)) {
    rlang::abort(message = paste0(
      "Invalid argument provided: listing_style\n", test_result
    ))
  }
  ...
  use_fallback <-
    dplyr::case_when(
      ...
      markdown_flavor == "gitlab" & !is_header_title & fallback_id_position > 0L ~ TRUE,
